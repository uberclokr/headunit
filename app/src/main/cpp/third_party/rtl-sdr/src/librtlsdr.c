...
/* Android (Helm patch): apps may not enumerate usbfs, so the fd that
 * UsbManager opened is registered here and the next rtlsdr_open() wraps it
 * via libusb_wrap_sys_device() instead of scanning the bus. One-shot. */
static int android_usb_fd = -1;
void rtlsdr_set_android_fd(int fd) { android_usb_fd = fd; }
...
int rtlsdr_open(rtlsdr_dev_t **out_dev, uint32_t index)
{
	...
	/* Helm patch: must be set before libusb_init or the fd wrap fails. */
	if (android_usb_fd >= 0)
	...
	/* Helm patch: wrap the Android-provided fd, skip discovery. */
	if (android_usb_fd >= 0) {
		r = libusb_wrap_sys_device(dev->ctx, android_usb_fd, &dev->devh);
		android_usb_fd = -1;
		if (r < 0 || !dev->devh)
			goto err;
		goto opened;
